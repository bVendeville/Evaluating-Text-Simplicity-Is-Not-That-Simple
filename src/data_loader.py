...
- lr_arts94.pkl, lr_arts300.pkl, lr_arts3k.pkl: Length-residualized ARTS datasets
...
- metric_scores: Dict[str, np.ndarray] mapping metric name to scores
  - 'human': ground truth human scores (or residualized scores for LR-ARTS)
...
def load_lr_arts94() -> Dict:
    """Load Length-Residualized ARTS94 dataset."""
    data = load_dataset('lr_arts94')
    print(f"  LR-ARTS94: {data['n_samples']} samples, {len(get_metric_names(data))} metrics [LENGTH-RESIDUALIZED]")
    return data


def load_lr_arts300() -> Dict:
    """Load Length-Residualized ARTS300 dataset."""
    data = load_dataset('lr_arts300')
    print(f"  LR-ARTS300: {data['n_samples']} samples, {len(get_metric_names(data))} metrics [LENGTH-RESIDUALIZED]")
    return data


def load_lr_arts3k() -> Dict:
    """Load Length-Residualized ARTS3k dataset."""
    data = load_dataset('lr_arts3k')
    print(f"  LR-ARTS3k: {data['n_samples']} samples, {len(get_metric_names(data))} metrics [LENGTH-RESIDUALIZED]")
    return data
...
def load_lr_arts_datasets() -> Dict[str, Dict]:
    """Load all Length-Residualized ARTS datasets."""
    return {
        'LR-ARTS94': load_lr_arts94(),
        'LR-ARTS300': load_lr_arts300(),
        'LR-ARTS3k': load_lr_arts3k(),
    }
...
def load_all_datasets() -> Dict[str, Dict]:
    ...
    datasets.update(load_lr_arts_datasets())
